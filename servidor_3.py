...
def preparar_compra():
    ...
    with lock:
        ...
        if sucesso:
            ...
            try:
                prepare_responses = []
                for url in [SERVER_2_URL, SERVER_1_URL]:
...
def buscar_rotas():
    ...
    trechos_viagem = carregar_trechos()
    servidores_externos = [SERVER_2_URL, SERVER_1_URL]
...
@app.route('/abortar_compra', methods=['POST'])
def abortar_compra():
    data = request.get_json()
    cpf = data.get('cpf')
    ...
    # Lógica para reverter as ações se necessário
    return jsonify({"msg": "Compra abortada"}), 200
